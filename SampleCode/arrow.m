...
oldaxlims(min(find(oldaxlims(:,1)==0)),:) = [ii reshape(axl',1,6)]; % 	oldaxlims(min(find(oldaxlims(:,1)==0)),:) = [curax reshape(axl',1,6)];
...
%	% additional properties
	...
	if isempty(oldaxlims),
        ...
        lims = get(ax(oldaxlims(:,1)),{'XLim','YLim','ZLim'})';    % 		lims = get(oldaxlims(:,1),{'XLim','YLim','ZLim'})';
		lims = reshape(cat(2,lims{:}),6,size(lims,2));
		mask = arrow_is2DXY(ax(oldaxlims(:,1)));   %mask = arrow_is2DXY(oldaxlims(:,1));
